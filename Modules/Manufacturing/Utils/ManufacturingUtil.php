...
class ManufacturingUtil extends Util
{
    ...
    public function getIngredientDetails($ingredient_array, $business_id, $location_id = null, $recipe = null, $unformat = false)
    {
        ...
        $with = ['product', 'product_variation', 'product.unit'];
        ...
        foreach ($ingredient_variations as $ingredient_variation) {
            ...
            $sub_units = $this->getSubUnits($business_id, $ingredient_variation->product->unit->id);
            $unit_name = $ingredient_variation->product->unit->short_name;
            $is_sub_unit = false;
            $sub_unit_id = null;
            $multiplier = 1;
            if (!empty($sub_units)) {
                foreach ($sub_units as $key => $value) {
                    if (!empty($ingredient_array[$ingredient_variation->id]['sub_unit_id']) && $ingredient_array[$ingredient_variation->id]['sub_unit_id'] == $key) {
                        $unit_name = $value['name'];
                        $sub_unit_id = $ingredient_array[$ingredient_variation->id]['sub_unit_id'];
            ...
            $total_price = $ingredient_variation->dpp_inc_tax * $line_total_quantity * $multiplier;
            $waste_percent = !empty($ingredient_array[$ingredient_variation->id]['waste_percent']) ? $ingredient_array[$ingredient_variation->id]['waste_percent'] : 0;

            $wasted_qty = $this->calc_percentage($line_total_quantity, $waste_percent);
            $final_quantity = $line_total_quantity - $wasted_qty;
            $ingredients[] = [
                'dpp_inc_tax' => $ingredient_variation->dpp_inc_tax,
                'quantity' => $line_total_quantity,
                'full_name' => $ingredient_variation->full_name,
                'id' => $ingredient_variation->id,
                'unit' => $unit_name,
                'allow_decimal' => $ingredient_variation->product->unit->allow_decimal,
                'variation' => $ingredient_variation,
                'enable_stock' => $ingredient_variation->product->enable_stock,
